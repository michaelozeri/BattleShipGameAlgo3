...
BoardFileErrorCode BoardValidator::ValidateBoard(Board3D boardToValidate)
{
		...
		for (int j = 0; j < cols; j++)
		{
			for (int k = 0; k < depth; k++)
			{
				...
				if (cellChar != 'X' && cellChar != BLANK)  //if i didnt check that place already and not BLANK
				{
					...
					{
						if (GameBoardUtils::IsPlayerIdChar(PlayerAID, cellChar))
						{
							playerAboatNum++; // Add valid ship for player A
						}
						else if (GameBoardUtils::IsPlayerIdChar(PlayerBID, cellChar))
						{
